#read and make 
x <- read.csv("C:/Users/Administrator/Documents/Movie_R_2/201707.txt", header = T, fileEncoding = "UTF-8-BOM", sep = "\t", fill = TRUE, quote="" )
names(x)
head(x, n=10) #데이터의 앞부분 10개 행 보기 (default n is 6)
...
#save wide_minus to do factor analysis
minus_a <- subset(minus, select = c(movieCd, D_day, audiCnt))
wide_minus_a<- dcast(minus_a, movieCd ~ D_day, sum)
setwd('C:/Users/Administrator/Documents/Movie_R_2/170820_EDM_PCA_Pearson')
write.csv(wide_minus_a, file = "wide_minus.csv", row.names = FALSE)

plus <- subset(x_2, D_day > 0)
plus_a <- subset(plus, select = c(movieCd, D_day, audiCnt))
plus_a
wide_plus_a <-dcast(plus_a, movieCd ~ D_day, sum)
wide_plus_a
write.csv(wide_plus_a, file = "wide_plus.csv", row.names = FALSE)
